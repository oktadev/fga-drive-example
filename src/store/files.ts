...
export async function getFiles(parent: string) {
  ...
  filesForFolder.forEach(fileId => promises.push(kv.hgetall(`file:${fileId}`)));

  return (await Promise.all(promises)).map((file, index) => ({...file, id: filesForFolder[index]}))
}
...
export async function getFilesSubset(subset: Array<string>) {
  ...
  return (await Promise.all(promises)).map((file, index) => ({...file, id: subset[index]}))
}

export async function createFile(fileId: string, parent: string, file: StoredFile) {
